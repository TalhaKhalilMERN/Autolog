...
import { useDeleteReminder } from "@/features/vehicles/hooks/use-reminders";

/**
 * DeleteReminderButton
 *
 * Client component that calls useDeleteReminder and invalidates the cache.
...
export function DeleteReminderButton({
      ...
      <div className="mt-2 space-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2">
          <p className="flex-1 text-xs text-foreground">
            Delete <span className="font-semibold">{title}</span> reminder?
          </p>
          <button
  ...
  return (
    <button
      ...
      className="flex items-center gap-1 rounded border border-destructive/20 px-2.5 py-1 text-xs font-medium text-destructive transition-all hover:bg-destructive/10 cursor-pointer"
